...
import { IDropdownOption, IIconProps, IconButton, Label, Panel, PrimaryButton, TextField, Modal, FontWeights, getTheme, mergeStyleSets } from '@fluentui/react';
...
export default class Dynamicsolution extends React.Component<IDynamicsolutionProps, IDynamicsolutionPropsState, {}> {
      ...
      .then(async (quiklinks: any) => {
        if (quiklinks.length > 0) {
          if (quiklinks[0].AudienceTargets !== null && quiklinks[0].AudienceTargets !== "") {
            quiklinks[0].AudienceTargets.split(',').forEach((item: any) => {
              let tempgroups = {
                value: item,
                label: item
              }
  ...
  public _onSelectQuicklinkType(event: React.FormEvent<HTMLDivElement>, option: IDropdownOption): void {
    this.setState({
      QuicklinktypeID: (option).toString(),
          ...
          {this.state.QuicklinktypeID === "Icon" &&
            <div className={styles.iconmain} >
              {this.state.quicklinkitems.map((cards: any, key: any) => {
                ...
                return (
                  <div key={key} className={styles.iconcard} style={{ backgroundColor: this.props.backgroundColor, width: this.props.btnBoxSize !== "" ? this.props.btnBoxSize : "8rem", height: this.state.Height !== "" ? this.state.Height : "11rem", display: display }} >
                    ...
                    <div>
                      <img className={styles.iconimg} style={{ width: this.props.btnIconSize !== "" ? this.props.btnIconSize : "50%", height: this.props.btnIconSize !== "" ? this.props.btnIconSize : "50%" }} src={cards.CardIcon.Url !== null ? cards.CardIcon.Url : "Image"} alt='' />
                    </div>
                    <div className={styles.texticondiv} >
                      <h5 className={styles.iconcardtitle} style={{ fontSize: this.props.btnTextSize, color: this.props.color }}>{cards.Title}</h5>
                      <a href={cards.link} target="_blank" className={styles.linkicon} ><IconButton title={cards.Title} className={styles.iconfrwd}
                        iconProps={{ iconName: 'CheckedOutByYou12' }} // Set iconName to 'None' to hide default Fluent UI icon
                        styles={{ root: { padding: 0, color: this.props.color } }} // Optional: Remove default padding                     
                        style={{ width: this.props.btnRedirectLinkSize !== "" ? this.props.btnRedirectLinkSize : 20, height: this.props.btnRedirectLinkSize !== "" ? this.props.btnRedirectLinkSize : 20 }}
                        ariaLabel="NavigateForward" onMouseEnter={() => this.handleHover(key)} onMouseLeave={this.handleHoverOut} >
                        {/* <img src={this.props.siteUrl + "/SiteAssets/New ICon.png"} alt="Custom icon" style={{ width:this.state.btnRedirectLinkSize !== "" ?this.state.btnRedirectLinkSize : 20, height:this.state.btnRedirectLinkSize !== "" ?this.state.btnRedirectLinkSize : 20 }} /> */}
                      </IconButton>
                      </a>
                    </div>
                  </div>)
          ...
          {this.state.QuicklinktypeID === "Image" &&
            <div className={styles.imagemain} >
              {this.state.quicklinkitems.map((Imagecards: any, key: any) => {
                ...
                return (<div key={key} className={styles.imagecard} style={{ backgroundColor: this.props.backgroundColor, width: this.props.btnBoxSize !== "" ? this.props.btnBoxSize : "26%", height: this.state.Height !== "" ? this.state.Height : "11rem", display: display }} >
                  ...
                  <div className={styles.imagecrdall} >
                    ...
                    <div className={styles.imgcrdtxticndiv} >
                      <h5 style={{ fontSize: this.props.btnTextSize, color: this.props.color }} >{Imagecards.Title}</h5> <a href={Imagecards.link} className={styles.imagcrdtxt} target="_blank"><IconButton title={Imagecards.Title} iconProps={key === isHovered ? RightDouble : RightIcon} style={{ float: "right" }} ariaLabel="ChevronRightSmall" onMouseEnter={() => this.handleHover(key)} onMouseLeave={this.handleHoverOut} /> </a>
                    </div>
                  </div>
                  {this.state.editAvlble &&
          ...
          {this.state.QuicklinktypeID === "Text" &&
            <div className={styles.textcardmain}  >
              {this.state.quicklinkitems.map((Textcards: any, key: any) => {
                ...
                return (<div className={styles.textcarddiv} style={{ backgroundColor: this.props.backgroundColor, width: this.props.btnBoxSize !== "" ? this.props.btnBoxSize : "45%", height: this.state.Height !== "" ? this.state.Height : "4rem", display: display }}>

                  <div className={styles.textcarditem} > <h5 style={{ fontSize: this.props.btnTextSize, color: this.props.color }}>{Textcards.Title}</h5> <a href={Textcards.link} target="_blank" style={{ float: "right", textDecoration: "none" }}><IconButton title={Textcards.Title} iconProps={key === isHovered ? RightDouble : RightIcon} className={styles.textcardlinkbtn} ariaLabel="ChevronRightSmall" onMouseEnter={() => this.handleHover(key)} onMouseLeave={this.handleHoverOut} /> </a> </div>
                  {this.state.editAvlble &&
              ...
              <AccordionPanel>
                {this.state.editCloase &&
                  <div className={styles.modalcontents}>
                    ...
                    {/* <Dropdown
                      onRenderPlaceholder="Select the Layout"
                      onRenderLabel="Type of Quicklinks"
